...
export const ViewProductModal: React.FC<ViewProductModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-950/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden border border-brand-200">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-brand-100 bg-brand-50/50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-brand-900">Detalle del Producto</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          {/* Large Image Preview */}
          <div className="w-full h-72 rounded-xl overflow-hidden bg-brand-100 border border-brand-200 relative group shadow-inner">
          ...
          <div className="space-y-4">
            <div>
              ...
              <h4 className="text-2xl font-black text-brand-900 mt-1">{product.name}</h4>
            </div>

            <div className="p-4 rounded-xl bg-brand-50 border border-brand-200">
              <p className="text-xs font-medium text-brand-500 mb-1">Descripción</p>
              <p className="text-sm text-brand-700 leading-relaxed whitespace-pre-line">
                {product.description}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-2">
              ...
              <div className="p-3.5 rounded-xl bg-brand-100/70 border border-brand-200 col-span-2 sm:col-span-1">
                <span className="text-xs text-brand-500 font-medium flex items-center gap-1">
                  <Calendar className="w-3.5 h-3.5" /> ID Registro
                </span>
                <p className="text-lg font-bold text-brand-800 mt-0.5">#{product.id}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-brand-100 bg-brand-50/50 flex justify-end">
          <button
            onClick={onClose}
            className="px-5 py-2 text-sm font-semibold text-brand-700 hover:text-brand-900 bg-white border border-brand-300 rounded-xl hover:bg-brand-50 transition-colors shadow-sm"
          >
            Cerrar
          </button>
        </div>
...
export const ProductFormModal: React.FC<ProductFormModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-950/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-xl bg-white rounded-2xl shadow-2xl overflow-hidden border border-brand-200">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-brand-100 bg-brand-50/50">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            <label className="block text-xs font-semibold text-brand-700 uppercase mb-1">
              Nombre del Producto *
            </label>
            <input
              type="text"
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent text-sm"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent text-sm bg-white"
            >
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent text-sm resize-none"
            />
          </div>
          ...
          <div className="pt-4 border-t border-brand-100 flex items-center justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-sm font-medium text-brand-700 hover:bg-brand-100 rounded-xl transition-colors"
            >
              Cancelar
            </button>
            <button
...
export const DeleteConfirmModal: React.FC<DeleteConfirmModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-950/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 border border-brand-200">
        <div className="w-12 h-12 rounded-2xl bg-rose-100 text-rose-600 flex items-center justify-center mb-4">
          <AlertTriangle className="w-6 h-6" />
        </div>

        <h3 className="text-lg font-bold text-brand-900 mb-2">¿Eliminar este producto?</h3>
        <p className="text-sm text-brand-600 leading-relaxed mb-6">
          Estás a punto de eliminar permanentemente <span className="font-semibold text-brand-900">&quot;{productName}&quot;</span>. Esta acción no se puede deshacer.
        </p>
