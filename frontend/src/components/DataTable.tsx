...
export const DataTable: React.FC<DataTableProps> = ({
  ...
  return (
    <div className="w-full bg-white rounded-2xl shadow-sm border border-ink-200 overflow-hidden">
      {/* Table Toolbar */}
      <div className="p-5 border-b border-ink-200 flex flex-col sm:flex-row items-center justify-between gap-4 bg-ink-50/50">
        ...
        <div className="relative w-full sm:w-96">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-ink-300 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all"
          />
          {searchTerm && (
            <button
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm text-ink-600">
          <thead className="bg-brand-900 text-brand-50 text-xs uppercase font-bold tracking-wider border-b border-brand-800">
            <tr>
          ...
          <tbody className="divide-y divide-ink-200">
            {filteredProducts.length > 0 ? (
              filteredProducts.map((product) => (
                <tr
                  key={product.id}
                  className="even:bg-ink-50/70 hover:bg-accent-50 transition-colors group"
                >
                  {/* Name & Thumbnail */}
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-xl overflow-hidden bg-ink-100 border border-ink-200 shrink-0 relative">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="inline-flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className="p-2 text-ink-600 hover:text-brand-600 hover:bg-brand-50 rounded-lg transition-colors"
                      >
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className="p-2 text-ink-600 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors"
                        >
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className="p-2 text-ink-600 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors"
                        >
