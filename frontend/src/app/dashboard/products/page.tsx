...
export default function ProductsPage() {
  ...
  return (
    <div className="px-5 pb-8 pt-2 sm:p-10 sm:pt-2 space-y-8 max-w-7xl w-full mx-auto">
      {/* Toast alert */}
      {toast && (
        <div
          className={`fixed bottom-6 right-6 z-50 flex items-center gap-2.5 px-5 py-3 rounded-2xl shadow-xl border text-sm font-medium animate-fade-in-up ${
            toast.type === "success"
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-surface-500 text-xs font-semibold uppercase tracking-wider mb-1">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2.5 rounded-xl border border-surface-300 bg-white hover:bg-surface-50 text-surface-600 transition-colors shadow-sm disabled:opacity-50"
          >
            <RefreshCw className={`w-4 h-4 ${loading ? "animate-spin text-primary-600" : ""}`} />
          </button>

          <div className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white border border-surface-200 shadow-sm text-xs font-semibold text-surface-700">
            {isAdmin ? (
