...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (new URLSearchParams(window.location.search).get("reason") === "inactive") {
      setNotice("Tu sesión se cerró por inactividad. Inicia sesión para continuar.");
    }
  }, []);
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 bg-slate-950 text-slate-100 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-6">
        ...
        {notice && (
          <div className="p-3 bg-amber-500/10 border border-amber-500/30 rounded-xl text-xs text-amber-200" role="status">
            {notice}
          </div>
        )}
